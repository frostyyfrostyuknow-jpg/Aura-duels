...
local THEME = {
	...
	Section = Color3.fromRGB(16, 6, 10),
	Card = Color3.fromRGB(18, 7, 12),
	Accent = Color3.fromRGB(255, 20, 100),
	AccentDark = Color3.fromRGB(200, 10, 60),
	Text = Color3.fromRGB(255, 80, 140),
	DarkText = Color3.fromRGB(220, 40, 100),
	Outline = Color3.fromRGB(255, 20, 100),
	SliderTrack = Color3.fromRGB(22, 8, 14),
	InputBg = Color3.fromRGB(20, 7, 12),
	ToggleOff = Color3.fromRGB(50, 15, 25),
	FloatButton = Color3.fromRGB(8, 3, 6),
	ProgressBg = Color3.fromRGB(10, 4, 6),
	ProgressFill = Color3.fromRGB(255, 20, 100),
}
...
-- ============================================================
--  AUTO PLAY (NineHub Duel patrol system)
-- ============================================================
local autoPlayEnabled = false
local autoPlaySide = "none"       -- "left" or "right"
local autoPlayWaypoint = 1
local autoPlayLastTpSide = "none"
local autoPlayWaitingCountLeft = false
local autoPlayWaitingCountRight = false
local autoPlayRagdollActive = false
local autoPlayRagdollWas = false
local autoPlayHeartbeatConn = nil
local autoPlayRagdollConn = nil
local autoPlayCountdownConn = nil
local AUTO_START_DELAY = 0.70
local PATROL_SPEED_FAST = 60
local PATROL_SPEED_SLOW = 29.4
local RAGDOLL_TP_DELAY = 0.12
local RAGDOLL_RESTART_DELAY = 0.18
local AP_RIGHT_WAYPOINTS = {
	Vector3.new(-473.04, -6.99,  29.71),
	Vector3.new(-483.57, -5.10,  18.74),
	Vector3.new(-475.00, -6.99,  26.43),
	Vector3.new(-474.67, -6.94, 105.48),
}
local AP_LEFT_WAYPOINTS = {
	Vector3.new(-472.49, -7.00,  90.62),
	Vector3.new(-484.62, -5.10, 100.37),
	Vector3.new(-475.08, -7.00,  93.29),
	Vector3.new(-474.22, -6.96,  16.18),
}
local AP_TP_CHECK_A     = Vector3.new(-472.60, -7.00, 57.52)
local AP_TP_CHECK_LEFT  = Vector3.new(-472.65, -7.00, 95.69)
local AP_TP_CHECK_RIGHT = Vector3.new(-471.76, -7.00, 26.22)
local AP_TP_FINAL_LEFT  = Vector3.new(-483.59, -5.04, 104.24)
local AP_TP_FINAL_RIGHT = Vector3.new(-483.51, -5.10,  18.89)
-- ============================================================
...
local function createESP(plr)
	...
	hitbox.Color3 = Color3.fromRGB(255, 20, 100)
...
local function updateToggleUI(name, state)
	...
	btn.state = state
	btn.track.BackgroundColor3 = state and Color3.fromRGB(255, 20, 100) or Color3.fromRGB(50, 15, 25)
	...
	autoLeftConn = RunService.Heartbeat:Connect(function()
		...
		elseif autoLeftPhase == 2 then
			...
			if dist < 1 then
				...
				if floatButtons["AutoLeft"] then floatButtons["AutoLeft"].state = false floatButtons["AutoLeft"].indicator.BackgroundColor3 = Color3.fromRGB(50, 15, 25) end
				faceSouth()
				return
			end
			local dir = (POSITION_L2 - hrp.Position)
...
local function stopAutoLeft()
	...
	if floatButtons["AutoLeft"] then
		...
		floatButtons["AutoLeft"].indicator.BackgroundColor3 = Color3.fromRGB(50, 15, 25)
	end
end
	...
	autoRightConn = RunService.Heartbeat:Connect(function()
		...
		elseif autoRightPhase == 2 then
			...
			if dist < 1 then
				...
				if floatButtons["AutoRight"] then floatButtons["AutoRight"].state = false floatButtons["AutoRight"].indicator.BackgroundColor3 = Color3.fromRGB(50, 15, 25) end
				faceNorth()
				return
			end
			local dir = (POSITION_R2 - hrp.Position)
...
local function stopAutoRight()
	...
	if floatButtons["AutoRight"] then
		...
		floatButtons["AutoRight"].indicator.BackgroundColor3 = Color3.fromRGB(50, 15, 25)
	end
end
...
local function setAutoLeft(state)
	...
	if state and batAimbotToggled then
		...
		if floatButtons["BatAimbot"] then
			...
			floatButtons["BatAimbot"].indicator.BackgroundColor3 = Color3.fromRGB(50, 15, 25)
		end
	end
...
--  AUTO PLAY functions
-- ============================================================
local function apSafeTeleport(pos)
	local c = LocalPlayer.Character
	if not c then return end
...
local function apDoTPLeft()
	apSafeTeleport(AP_TP_CHECK_A)    task.wait(0.08)
	apSafeTeleport(AP_TP_CHECK_LEFT) task.wait(0.08)
	apSafeTeleport(AP_TP_FINAL_LEFT)
	autoPlayLastTpSide = "left"
end

local function apDoTPRight()
	apSafeTeleport(AP_TP_CHECK_A)     task.wait(0.08)
	apSafeTeleport(AP_TP_CHECK_RIGHT) task.wait(0.08)
	apSafeTeleport(AP_TP_FINAL_RIGHT)
	autoPlayLastTpSide = "right"
end

local function apIsRagdolled(c)
	if not c then return false end
	local h2 = c:FindFirstChildOfClass("Humanoid")
	if h2 and h2:GetState() == Enum.HumanoidStateType.Ragdoll then return true end
	for _, v in ipairs(c:GetChildren()) do
		if v.Name:lower():find("ragdoll") and v:IsA("BoolValue") and v.Value then return true end
	end
	return false
end

local function apStartPatrol(side)
	autoPlaySide = side
	autoPlayWaypoint = 1
end

local function apStopPatrol()
	autoPlaySide = "none"
	autoPlayWaypoint = 1
	autoPlayWaitingCountLeft = false
	autoPlayWaitingCountRight = false
end

local function apStartRagdollRecovery()
	if autoPlayRagdollConn then autoPlayRagdollConn:Disconnect() end
	autoPlayRagdollConn = RunService.Heartbeat:Connect(function()
		if not autoPlayRagdollActive then return end
		local c = LocalPlayer.Character
		if not c then return end
		local nowRag = apIsRagdolled(c)
		if nowRag and not autoPlayRagdollWas then
			autoPlayRagdollWas = true
			task.spawn(function()
				task.wait(RAGDOLL_TP_DELAY)
				if autoPlayLastTpSide == "left" then
					apDoTPLeft() task.wait(RAGDOLL_RESTART_DELAY)
					if autoPlaySide ~= "left" then apStartPatrol("left") end
				elseif autoPlayLastTpSide == "right" then
					apDoTPRight() task.wait(RAGDOLL_RESTART_DELAY)
					if autoPlaySide ~= "right" then apStartPatrol("right") end
				else
					apDoTPLeft() task.wait(RAGDOLL_RESTART_DELAY)
					apStartPatrol("left")
				end
		...
		elseif not nowRag then
			autoPlayRagdollWas = false
		end
	end)
end

local function apUpdateMovement()
	local c = LocalPlayer.Character
	if not c then return end
	local h = c:FindFirstChild("HumanoidRootPart")
	if not h then return end
	if autoPlaySide == "none" then return end
	local waypoints = (autoPlaySide == "right") and AP_RIGHT_WAYPOINTS or AP_LEFT_WAYPOINTS
	if #waypoints == 0 then return end
	local target = waypoints[autoPlayWaypoint]
	local pos = h.Position
	local distXZ = Vector3.new(target.X - pos.X, 0, target.Z - pos.Z).Magnitude
	if distXZ > 3.2 then
		local dir = (Vector3.new(target.X, 0, target.Z) - Vector3.new(pos.X, 0, pos.Z)).Unit
		local spd = (autoPlayWaypoint >= 3) and PATROL_SPEED_SLOW or PATROL_SPEED_FAST
		h.AssemblyLinearVelocity = Vector3.new(dir.X * spd, h.AssemblyLinearVelocity.Y, dir.Z * spd)
	else
		if autoPlayWaypoint >= #waypoints then
			apStopPatrol()
			task.delay(0.35, function()
				if autoPlayEnabled then
					apStartPatrol(autoPlayLastTpSide ~= "none" and autoPlayLastTpSide or "left")
				end
			end)
		else
			autoPlayWaypoint = autoPlayWaypoint + 1
		end
	end
end

local function apWatchCountdown()
	if autoPlayCountdownConn then autoPlayCountdownConn:Disconnect() autoPlayCountdownConn = nil end
	local label
	pcall(function()
		local top = LocalPlayer.PlayerGui:FindFirstChild("DuelsMachineTopFrame", true)
		if not top then return end
		local timer = top:FindFirstChild("Timer", true)
		if not timer then return end
		label = timer:FindFirstChild("Label", true)
	end)
	if not label then return end
	autoPlayCountdownConn = label:GetPropertyChangedSignal("Text"):Connect(function()
		local num = tonumber(label.Text)
		if num and num == 1 then
			task.wait(AUTO_START_DELAY)
			if autoPlayWaitingCountLeft then
				autoPlayWaitingCountLeft = false
				apStartPatrol("left")
			end
			if autoPlayWaitingCountRight then
				autoPlayWaitingCountRight = false
				apStartPatrol("right")
			end
		end
...
local function startAutoPlay()
	...
	autoPlayRagdollActive = true
	apStartRagdollRecovery()
	if autoPlayHeartbeatConn then autoPlayHeartbeatConn:Disconnect() end
	autoPlayHeartbeatConn = RunService.Heartbeat:Connect(apUpdateMovement)
	autoPlayWaitingCountLeft = true
	apWatchCountdown()
	-- TP to left side immediately so ragdoll recovery knows the side
	task.spawn(function()
		task.wait(0.1)
		apDoTPLeft()
	end)
...
local function stopAutoPlay()
	...
	autoPlayRagdollActive = false
	apStopPatrol()
	if autoPlayHeartbeatConn then autoPlayHeartbeatConn:Disconnect() autoPlayHeartbeatConn = nil end
	if autoPlayRagdollConn then autoPlayRagdollConn:Disconnect() autoPlayRagdollConn = nil end
	if autoPlayCountdownConn then autoPlayCountdownConn:Disconnect() autoPlayCountdownConn = nil end
	local c = LocalPlayer.Character
	if c then
		local h = c:FindFirstChild("HumanoidRootPart")
...
local function setupChar(c)
	...
	if head then
		...
		speedLbl.TextColor3 = Color3.fromRGB(255, 20, 100)
...
HUDContainer.Size = UDim2.new(0, s(300), 0, s(30))
HUDContainer.Position = UDim2.new(0.5, -s(150), 0, s(8))
HUDContainer.BackgroundColor3 = Color3.fromRGB(10, 3, 6)
HUDContainer.BackgroundTransparency = 0.1
HUDContainer.BorderSizePixel = 0
HUDContainer.Visible = true
Instance.new("UICorner", HUDContainer).CornerRadius = UDim.new(0, s(10))
local HUDStroke = Instance.new("UIStroke", HUDContainer)
HUDStroke.Color = Color3.fromRGB(255, 20, 100)
HUDStroke.Thickness = 1.5

local HUDTitle = Instance.new("TextLabel", HUDContainer)
HUDTitle.Size = UDim2.new(0, s(100), 1, 0)
HUDTitle.Position = UDim2.new(0, s(6), 0, 0)
HUDTitle.BackgroundTransparency = 1
HUDTitle.Text = "âœ¦ AURA HUB"
HUDTitle.TextColor3 = Color3.fromRGB(255, 20, 100)
HUDTitle.Font = Enum.Font.GothamBold
HUDTitle.TextSize = s(11)
HUDTitle.TextStrokeTransparency = 0.5
HUDTitle.TextStrokeColor3 = Color3.fromRGB(100, 0, 30)
HUDTitle.TextXAlignment = Enum.TextXAlignment.Left

FPSLabel = Instance.new("TextLabel", HUDContainer)
FPSLabel.Size = UDim2.new(0, s(70), 1, 0)
FPSLabel.Position = UDim2.new(0.5, -s(35), 0, 0)
FPSLabel.BackgroundTransparency = 1
FPSLabel.Text = "FPS: 0"
FPSLabel.TextColor3 = Color3.fromRGB(255, 90, 150)
FPSLabel.Font = Enum.Font.GothamBold
FPSLabel.TextSize = s(11)
FPSLabel.TextStrokeTransparency = 0.5
FPSLabel.TextStrokeColor3 = Color3.fromRGB(100, 0, 30)
FPSLabel.TextXAlignment = Enum.TextXAlignment.Center

PingLabel = Instance.new("TextLabel", HUDContainer)
PingLabel.Size = UDim2.new(0, s(80), 1, 0)
PingLabel.Position = UDim2.new(1, -s(86), 0, 0)
PingLabel.BackgroundTransparency = 1
PingLabel.Text = "Ping: 0ms"
PingLabel.TextColor3 = Color3.fromRGB(255, 90, 150)
PingLabel.Font = Enum.Font.GothamBold
PingLabel.TextSize = s(11)
PingLabel.TextStrokeTransparency = 0.5
PingLabel.TextStrokeColor3 = Color3.fromRGB(100, 0, 30)
...
Title.Text = "AURA HUB" Title.TextColor3 = THEME.Accent Title.Font = Enum.Font.GothamBold
...
local function createToggle(name, keyHintPC, keyHintController, defaultState, callback, order)
	...
	Track.BackgroundColor3 = defaultState and Color3.fromRGB(255, 20, 100) or Color3.fromRGB(50, 15, 25) Track.BorderSizePixel = 0
	...
	Btn.MouseButton1Click:Connect(function()
		...
		Track.BackgroundColor3 = ns and Color3.fromRGB(255, 20, 100) or Color3.fromRGB(50, 15, 25)
...
local function createCombatKeybindBadge(parent, actionName)
	...
	badge.BackgroundColor3 = Color3.fromRGB(255, 20, 100)
	...
	badge.MouseButton1Click:Connect(function()
		...
		conn = UserInputService.InputBegan:Connect(function(input)
			...
			if input.UserInputType == Enum.UserInputType.Keyboard then
				KEYBINDS[actionName].PC = input.KeyCode
				badge.BackgroundColor3 = Color3.fromRGB(255, 20, 100) listeningForKeybind = nil
				saveConfig() syncKeybindDisplays(actionName) conn:Disconnect()
			end
		end)
...
createToggle("Auto Play", "?", "?", autoPlayEnabled, function(v)
